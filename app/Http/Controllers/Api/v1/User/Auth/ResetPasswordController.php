...
class ResetPasswordController extends Controller
{
    ...
    public function sendResetOtp(Request $request)
    {
        ...
        if ( $validation->fails() ) {
            ...
            $user = User::where('phone_number',$input['phone_number'])->where('deleted_at',null)->first();
            if($user){
                $todayDate=date("Y-m-d H:i:s");
                $expirtDateTime= date("Y-m-d H:i:s",strtotime("+5 minutes", strtotime($todayDate)));
                
                $user->OTP = CommonHelpers::generateOtp(6);
                $user->expiry_time=$expirtDateTime;
                $user->save();
                
                $message = array('success'=>1,'OTP'=>$user->OTP,'message'=>'Please check your phone number for OTP', 'otp_expiry_time'=> $expirtDateTime );
    ...
    {       
        ...
        $validation = Validator::make($input,$messages);
        ...
        if ( $validation->fails() ) {
            ...
            if( $user ){
                ...
                $message = array('success'=>0,'message'=>'User account not found.');
                return  response()->json($message);
            }
        }
    }
        ...
        if ( $validation->fails() ) {
            ...
            if( $user ){
                if( $user->OTP == $input['otp'] ){
                    if( $user->expiry_time >=  date("Y-m-d H:i:s") ){
                        
                        $user->otp = null;
                        $user->expiry_time = null;
                        $user->save();
                        
                        $message = array('success'=>1,'message'=>'OTP verified successfully');
                        return  response()->json($message);
